...
def render_result(address, route_matches, source_results):
    lines = [f"IP: {address}"]

    if route_matches:
        lines.append("Generated routes: yes")
        for network in route_matches:
            lines.append(f"  - {network}")
    else:
        lines.append("Generated routes: no")
    ...
    if source_results:
        ...
        for source, matches in source_results:
            ...
            for network in matches[:5]:
                lines.append(f"    match: {network}")
            if len(matches) > 5:
                lines.append(f"    ... and {len(matches) - 5} more")
    else:
        lines.append("Sources: no cache match found")
