...
Перед применением нового `generated/routes.conf` контейнер оставляет копию предыдущего файла. Обновление выполняется по принципу "все или ничего": если не удалось скачать хотя бы один источник, получить префиксы ASN, обработать Google ranges или зарезолвить домен из include/exclude, старый файл маршрутов остается на месте. Если `birdc configure` не принимает обновленную конфигурацию, `entrypoint.sh` восстанавливает старый файл маршрутов и повторно просит BIRD применить рабочий вариант.
